...
DROP POLICY IF EXISTS "Users can view their AI generations" ON ai_generations;
CREATE POLICY "Users can view their AI generations" 
ON ai_generations FOR SELECT 
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can create AI generations" ON ai_generations;
CREATE POLICY "Users can create AI generations" 
ON ai_generations FOR INSERT 
WITH CHECK (user_id = auth.uid());

-- Create RLS policies for browser_sessions
DROP POLICY IF EXISTS "Users can view their browser sessions" ON browser_sessions;
CREATE POLICY "Users can view their browser sessions" 
ON browser_sessions FOR SELECT 
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can manage their browser sessions" ON browser_sessions;
CREATE POLICY "Users can manage their browser sessions" 
ON browser_sessions FOR ALL 
USING (user_id = auth.uid())
WITH CHECK (user_id = auth.uid());

-- Create RLS policies for visual test flows
DROP POLICY IF EXISTS "Users can view project visual test flows" ON visual_test_flows;
CREATE POLICY "Users can view project visual test flows" 
ON visual_test_flows FOR SELECT 
USING (
    project_id IN (
...
DROP POLICY IF EXISTS "Users can manage project visual test flows" ON visual_test_flows;
CREATE POLICY "Users can manage project visual test flows" 
ON visual_test_flows FOR ALL 
USING (
    project_id IN (
        SELECT p.id FROM projects p
        JOIN organization_members om ON p.organization_id = om.organization_id
...
-- Visual test steps inherit permissions from flows
DROP POLICY IF EXISTS "Users can view visual test steps" ON visual_test_steps;
CREATE POLICY "Users can view visual test steps" 
ON visual_test_steps FOR SELECT 
USING (
    flow_id IN (
        SELECT id FROM visual_test_flows
        WHERE project_id IN (
            SELECT p.id FROM projects p
            JOIN organization_members om ON p.organization_id = om.organization_id
            WHERE om.user_id = auth.uid() AND om.status = 'active'
...
DROP POLICY IF EXISTS "Users can manage visual test steps" ON visual_test_steps;
CREATE POLICY "Users can manage visual test steps" 
ON visual_test_steps FOR ALL 
USING (
    flow_id IN (
        SELECT id FROM visual_test_flows
        WHERE project_id IN (
